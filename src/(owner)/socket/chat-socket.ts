...
export interface OwnerChat {
  ...
  booking_status: "active" | "checked_out" | "upcoming";
}
...
class ChatSocket {
    ...
    try {
      ...
      return new Promise((resolve) => {
        ...
        this.socket!.once("connect", () => {
          ...
          console.log(
            `[FRONTEND-SOCKET] ${this.userRole} chat socket connected with userId: ${this.currentUserId}`
          );
  ...
  onMessageSent(
    handler: (confirmation: MessageSentConfirmation) => void
  ): () => void {
  ...
  private setupEventListeners(): void {
    ...
    this.socket.on("joined", (data: any) => {
      console.log(
        `[FRONTEND-SOCKET] ${this.userRole} joined successfully to user_${data.userId}:`,
        data
      );
    });
    ...
    this.socket.on("new_chat", (chatData: any) => {
  ...
  private emitChatUpdate(update: ChatUpdateData): void {
    console.log(
      `[FRONTEND-SOCKET] Emitting chat update to ${this.chatUpdateHandlers.size} handlers:`,
      update
    );
