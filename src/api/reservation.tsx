...
export const reservationAPI = {
  ...
  checkAvailability: async (
    ...
    try {
      ...
      throw new Error(
        error.message ||
          "Failed to check room availability. Please try again later."
      );
    }
  },
  ...
  createReservation: async (
    ...
    try {
      ...
      if (error.status === 404) {
        throw new Error(
          "Room or resort not found. Please try selecting a different room."
        );
      }
      if (error.status === 409) {
        throw new Error(
          "Room is no longer available for the selected dates. Please choose different dates."
        );
      }
      if (error.status === 400) {
        throw new Error(
          "Invalid booking information. Please check your dates and try again."
        );
      }
      if (error.status === 401) {
      ...
      throw new Error(
        error.message || "Failed to create reservation. Please try again later."
      );
    }
  },
  ...
  getOwnerReservations: async (
    params?: OwnerReservationsParams
    ...
    try {
      ...
      const url = `/reservation/owner-reservations${
  ...
  completeReservation: async (
    ...
    try {
      const response = await authenticatedApiRequest(
        `/reservation/${reservationId}/complete`,
        {
          method: "PUT",
        }
      );
  ...
  autoCompleteReservations: async (): Promise<{
    message: string;
